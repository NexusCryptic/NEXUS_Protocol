...
1. a stable record ID;
2. an observed timestamp;
3. source repository/conversation/file;
4. exact path where applicable;
5. commit SHA when GitHub-backed;
6. Git blob SHA when available;
7. SHA-256 when the raw artifact can be hashed;
8. ISO/BEAT verification fields when those project-specific checks are supplied;
